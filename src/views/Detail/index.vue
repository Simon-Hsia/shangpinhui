          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="item in spuSaleAttrList" :key="item.id">
                ...
                <dd
                  changepirce="0"
                  :class="{ active: child.isChecked === '1' }"
                  v-for="child in item.spuSaleAttrValueList"
                  :key="child.id"
                  @click="changeItem(child, item.spuSaleAttrValueList)"
                >
                  {{ child.saleAttrValueName }}
                </dd>
              </dl>
            </div>
...
export default {
  ...
  computed: {
    ...
    // 为了子组件把这个图片列表单独引出来，赋一个默认值，不然zoom子组件渲染的时候发现是unfined[0]就会飘红
    skuImageList() {
  ...
  methods: {
    changeItem(spuSaleAttrValue, list) {
      // 排他思想
